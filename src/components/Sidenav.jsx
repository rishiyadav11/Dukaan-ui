...
const Sidenav = () => {
  ...
  return (
    <div className="">
      <div className={`fixed pb-3 bg-[#1E2640] pt-5 px-2 left-0 top-0 w-56  min-h-screen z-50 flex flex-col ${click===true ? "translate-x-0" : "-translate-x-full"} gap-6 transition-transform -translate-x-full sm:translate-x-0  `}>
        <div className="upper  w-full flex items-center justify-between">
          ...
          <MdOutlineKeyboardArrowDown className=" text-4xl hidden sm:block text-white" />
          <RxCrossCircled onClick={()=>setclick(false)} className={`text-4xl text-white  sm:hidden ${!click && "rotate-180 transition-all duration-75"}  `}/>
        </div>
        <div className="lower flex flex-col gap-2 text-gray-300 text-lg font-semibold ">
          {Navlist.map((elem,id) => (
            <div key={id} className="w-full cursor-pointer bg-grey-100  hover:bg-gray-600 hover:text-white  flex gap-2  items-center px-4 rounded-md h-10 select-none">
              <h1>{elem.icon}</h1>
              <h1>{elem.name}</h1>
            </div>
          ))}
        </div>
        <div className=" flex items-center mt-auto p-2 gap-2.5 bg-[#353C53] rounded-lg  hover:bg-[#353c53a7] text-gray-400 hover:text-gray-200 cursor-pointer select-none">
      ...
      <div className=" bg-white">
        <button
          ...
          type="button"
          className="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden  hover:bg-white-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 hover:text-white-500 dark:focus:ring-gray-600"
        >
